...
def include_all(context, *args, **kwargs):
    ...
    # Include simulator/nav2 multi-robot simulation
    try:
    ...
    # Include per-robot automode launches sequentially: start next only after previous controller appears
    try:
        ...
        for i, ns in enumerate(robot_names):
            ...
            if i == 0:
                # start first include immediately, then start its waiter
                includes.append(include_action)
                includes.append(waiter)
            else:
                # start this include only after previous waiter's process exits
    ...
    # Add a timer node to shutdown after sim time limit
    # timer_node = Node(
    #     package='automode_tools',
    #     executable='timer_shutdown',
    #     name='timer_shutdown',
    #     output='screen',
    # )
    # includes.append(timer_node)

    # Add position collector node
    ...
    # Add aggregation scorer node (do that better in seperated terminal)
    # scorer_node = Node(
    #     package='automode_tools',
    #     executable='scorer_node_aggregation',
    #     name='aggregation_scorer',
    #     output='screen',
    # )
    # includes.append(scorer_node)
...
def generate_launch_description():
    ...
    config_file_arg = DeclareLaunchArgument(
        'config_file', default_value='config_aggregation.yaml',
